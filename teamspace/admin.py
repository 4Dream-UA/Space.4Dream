    ...
    def clean(self):
        ...
        if self.instance.pk and self.instance.role == Worker.Role.ADMIN:
            if role != Worker.Role.ADMIN or not is_active:
                admin_count = (
                    Worker.objects.filter(role=Worker.Role.ADMIN, is_active=True)
...
class WorkerAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        ("Operational data", {"fields": ("role", "position")}),
        (
    ...
    def delete_model(self, request, obj):
        if obj.role == Worker.Role.ADMIN:
            admin_count = Worker.objects.filter(
                role=Worker.Role.ADMIN, is_active=True
            ).count()
            if admin_count <= 1:
                messages.error(
                    request, "Security error: The last administrator cannot be deleted!"
                )
                return
    ...
    def delete_queryset(self, request, queryset):
        ...
        if admins_to_delete.exists():
            total_admins = Worker.objects.filter(
                role=Worker.Role.ADMIN, is_active=True
            ).count()
            if total_admins - admins_to_delete.count() < 1:
                messages.error(
                    request,
                    "Security warning: This action will remove all administrators. Operation canceled.",
                )
                return
...
class SystemSettingAdmin(admin.ModelAdmin):
    list_display = ["__str__", "log_retention_days"]
